  ...
  void goTo(int index) async {
    ...
    if (index == 0) {
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) =>
                  MyThoughtsPage(state.user, myThoughtsList)));
      checkLimits();
    }
    if (index == 1) {
      List<Meme> myMemesList = await memeService.getMemes(user.uid.toString());
      Navigator.push(
          state.context,
          MaterialPageRoute(
    ...
    if (index == 2) {
      ...
      Navigator.push(
          state.context,
          MaterialPageRoute(
              builder: (context) => SnapshotPage(state.user, imagelist)));
      checkLimits();
    }
    if (index == 3) {
      print('music');
